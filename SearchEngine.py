...
class SearchEngine:
    ...
    def query(self, input_query, num_results):
        ...
        print(out)  # don't delete
        return [self._urls[index] for index in result_urls]
    ...
    def _snippet(self,index,quoted,unquoted):
        ...
        # make it look nice
        words = best_txt.split()
        word_per_line = int(len(words) / 3 + 1)
        lines = [' '.join(words[i * word_per_line:(i + 1) * word_per_line]) for i in range(3)]
